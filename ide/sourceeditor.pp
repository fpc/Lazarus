...
{ TSourceEditor ---
  ...
  private
    ...
    procedure UpdateCompilerStatesFromCodeTools(Force: Boolean = False);
  protected
      ...
      then begin
        ...
        for i := 0 to FSharedEditorList.Count - 1 do begin
          ...
          if SharedEdit.Visible then
            SharedEdit.UpdateCompilerStatesFromCodeTools(True);
        end;
      end;
      for i := 0 to FSharedEditorList.Count - 1 do begin
        SharedEdit:=SharedEditors[i];
    ...
    if SrcLogEntry<>nil then begin
      ...
      for i := 0 to FSharedEditorList.Count - 1 do begin
        ...
        if SharedEditors[i].Visible then
          SharedEditors[i].UpdateCompilerStatesFromCodeTools(True);
      end;
    end;
...
begin
  ...
  try
    ...
    if (OldHlIsPas <> HlIsPas) then begin
      ...
      if Visible then
        UpdateCompilerStatesFromCodeTools(True);
    end
...
procedure TSourceEditor.UpdateCompilerStatesFromCodeTools(Force: Boolean);
...
begin
  ...
  try
    ...
    begin
      ...
      if EditorComponent.IsIfdefMarkupActive
      and (aDirective^.Kind in (lsdkAllIf+lsdkAllElse)) then
      begin
...
begin
  ...
  for i := 0 to SrcEdit.SharedEditorCount - 1 do
    SrcEdit.SharedEditors[i].UpdateCompilerStatesFromCodeTools(True);
  ...
  try
    ...
    begin
      if not SrcEdit.Visible then begin
        ...
        SrcEdit.UpdateCompilerStatesFromCodeTools; // after editor is initialized
      end;
...
begin
  ...
  for i:=0 to SourceEditorCount-1 do begin
    ...
    SrcEdit.UpdateCompilerStatesFromCodeTools;
  end;
end;
