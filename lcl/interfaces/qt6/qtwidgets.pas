...
type
  ...
  private
    ...
    function GetWindowHandle: QWindowH;
...
begin
  ...
  if DeltaPt.y <> 0 then
    Msg.WheelDelta := SmallInt(DeltaPt.y)
  else
    Msg.WheelDelta := SmallInt(DeltaPt.x);
  ...
  {$warning for Cocoa use QWheelEvent_pixelDelta(), see Qt6 docs}
  {$IFDEF DARWIN}
...
function TQtWidget.GetWindowHandle: QWindowH;
begin
...
var
  ...
  begin
    ...
    try
      ...
      //writeln(Format('>BtnM %d FocV %d FocH %d ShV %d Shh %d Result.cx %d cy %d',[BtnMargin, FocusV, FocusH, ShiftV, ShiftH, Result.cx, Result.cy]));
      Result.cx := Result.cx + BtnMargin + (FocusH * 2) + (ShiftH * 2);
      Result.cy := Result.cy + BtnMargin + (FocusV * 2) + (ShiftV * 2);
      // writeln(Format('<BtnM %d FocV %d FocH %d ShV %d Shh %d Result.cx %d cy %d Class %s',[BtnMargin, FocusV, FocusH, ShiftV, ShiftH, Result.cx, Result.cy, LCLObject.ClassName]));
...
var
  ...
  Color: TQColor;
  Painter: QPainterH;
  APoint, APos, AOldPos, ANewPos, AGlobalPos, APixelDelta: TQtPoint;
  APosF, AGlobalPosF: TQtPointF;
  AEvent: QEventH;
...
begin
  ...
  case QEvent_type(Event) of
    ...
    QEventWheel:
    begin
      {$note check this for Qt6}
      QWheelEvent_globalPosition(QWheelEventH(Event), @AGlobalPosF);
      QWheelEvent_pos(QWheelEventH(Event), @APos);
      QWidget_mapToParent(Widget, @APoint, @APos);
      APosF.X := APoint.X;
      APosF.Y := APoint.Y;
      QWheelEvent_angleDelta(QWheelEventH(Event), @APos);
      QWheelEvent_pixelDelta(QWheelEventH(Event), @APixelDelta);
      AEvent := QWheelEvent_Create(@APosF, @AGlobalPosF, @APixelDelta, @APos, QWheelEvent_buttons(QWheelEventH(Event)),
        QInputEvent_modifiers(QInputEventH(Event)), QWheelEvent_phase(QWheelEventH(Event)),  QWheelEvent_inverted(QWheelEventH(Event)),
        QWheelEvent_source(QWheelEventH(Event)));
      try
...
begin
  ...
  begin
    ...
    begin
      ...
      if p <> nil then
        QMdiArea_setBackground(QMdiAreaH(MdiAreaHandle.Widget), QPalette_base(P));
...
begin
  ...
  {$ENDIF}
  {$IFDEF HASX11}
  if IsX11 and (QtWidgetSet.WindowManagerName = 'xfwm4') and not IsMDIChild and
  ...
  begin
    if IsX11 and (QtWidgetSet.WindowManagerName = 'metacity') and not IsMDIChild then
        X11Raise(QWidget_winID(Widget))
    else
      QWidget_raise(Widget);
  end;
...
begin
  {$warning fixme Qt6}
  AMargins := QMargins_Create();
  QLineEdit_textMargins(QLineEditH(Widget), AMargins);
  Result.Left := QMargins_left(AMargins);
  Result.Top := QMargins_top(AMargins);
  Result.Right := QMargins_right(AMargins);
  Result.Bottom := QMargins_bottom(AMargins);
  QMargins_Destroy(AMargins);
end;
...
begin
  ...
  if QTextBlock_isValid(ATextBlock) then
  begin
    ACursor := QTextCursor_Create(ATextBlock);
    QTextCursor_select(ACursor, QTextCursorBlockUnderCursor);
    QTextCursor_removeSelectedText(ACursor);
    QTextCursor_deleteChar(ACursor);
...
begin
  ...
  try
    ...
    begin
      ...
      NMLV.iItem := AIndex;
      NMLV.iSubItem := ASubIndex;
      if QListWidgetItem_isSelected(Current) then
        NMLV.uNewState := LVIS_SELECTED
      else
    ...
    begin
      ...
      NMLV.iSubItem := ASubIndex;
      {$warning fixme Qt6}
      if QListWidgetItem_isSelected(Previous) then // QListWidget_isItemSelected(QListWidgetH(Widget), Previous) then
        NMLV.uNewState := LVIS_SELECTED
      else
        NMLV.uOldState := LVIS_SELECTED;
      NMLV.uChanged := LVIF_STATE;
...
begin
  if AItem <> nil then
    Result := QListWidgetItem_isSelected(AItem)
  else
    Result := False;
end;
...
begin
  if AItem = nil then
    ...
    Result := not QListWidgetItem_isHidden(AItem);
end;
...
begin
  if AItem <> nil then
    QListWidgetItem_setSelected(AItem, ASelect);
end;
...
begin
  if AItem <> nil then
    QListWidgetItem_setHidden(AItem, not AVisible);
end;
...
begin
  Result := not QTreeWidgetItem_isHidden(AItem);
end;
...
procedure TQtTreeWidget.setItemVisible(AItem: QTreeWidgetItemH;
  const AVisible: Boolean);
begin
  QTreeWidgetItem_setHidden(AItem, not AVisible);
end;
...
begin
  ...
  if not InUpdate and
    ...
    (QTreeWidgetItem_isSelected(AItem) = ASelect)) then
    exit;
  ...
  Msg.NMHdr := @NMLV.hdr;
  {$warning fixme Qt6}
  QTreeWidgetItem_setSelected(AItem, ASelect);
...
begin
  ...
  try
    ...
    begin
      ...
      if QTreeWidgetItem_isSelected(Previous) then
        NMLV.uNewState := LVIS_SELECTED
      else
        NMLV.uOldState := LVIS_SELECTED;
...
begin
  ...
  begin
    ...
    QPalette_setBrush(WP, QPaletteWindow, QPalette_window(P));
  end;
end;
...
begin
  ...
  {$ENDIF}
  {$IFDEF HASX11}
  if IsX11 and (QtWidgetSet.WindowManagerName = 'xfwm4') and (QApplication_activeModalWidget() <> nil) then
  begin
...
begin
  ...
  begin
    R := getGeometry;
    {$warning fixme Qt6 TQtHintWindow positioning multi monitor}
    (*
    ...
    *)
    Pt.x := R.Left;
    Pt.y := R.Top;
    ASibling := QScreen_virtualSiblingAt(QGuiApplication_primaryScreen, @Pt);
    if Assigned(ASibling) then
      QScreen_availableGeometry(ASibling, @D)
    else
      QScreen_availableGeometry(QGuiApplication_primaryScreen, @D);
...
begin
  {$warning fixme Qt6}
  QFileDialog_setOption(QFileDialogH(Widget), QFileDialogOptionDontConfirmOverwrite, not AValue);
...
begin
  ...
  QFileDialog_setOption(QFileDialogH(Widget), QFileDialogOptionReadOnly, AReadOnly);
end;
...
begin
  ...
  if not QFileDialog_testOption(QFileDialogH(Widget),
    QFileDialogOptionDontUseNativeDialog) then
    exit;
  FForwardBtn := nil;
  FBackBtn := nil;
  FUpBtn := nil;
...
begin
  ...
  {$IFDEF HASX11}
  if IsX11 and (QtWidgetSet.WindowManagerName = 'metacity') then
      X11Raise(QWidget_winID(Widget));
  {$ENDIF}
  {$IFDEF TQTMESSAGEBOXUSEPARENT}
  QWidget_activateWindow(Widget);
