...
type
  ...
  protected
    ...
    FFlags: set of (scfHasEdit, scfAfterKeyUp);
  ...
  private
    ...
    FBackSpaceAction: TSynCompletionBackSpaceAction;
  ...
  published
    property BackSpaceAction: TSynCompletionBackSpaceAction read FBackSpaceAction write FBackSpaceAction;
  ...
  private
    ...
    FWidth: Integer;
    function GetBackSpaceAction: TSynCompletionBackSpaceAction;
    ...
    procedure SetBackSpaceAction(AValue: TSynCompletionBackSpaceAction);
...
// added the VK_XXX codes to make it more readable / maintainable
    ...
    VK_BACK:
      if (Shift = []) then begin
        if (not (scfHasEdit in FFlags)) and
           ( (FBackSpaceAction in [cbsCancelIfNewlyOpened, cbsCancelIfNewlyOpenedOrPos0NoRepeat]) or
             ((FBackSpaceAction = cbsCancelIfPos0NewlyOpened) and (CurrentString=''))
           )
        then begin
          if Assigned(OnCancel) then OnCancel(Self);
        end
        ...
        if (CurrentString='') and
           ( (FBackSpaceAction = cbsCancelIfPos0) or
             ((FBackSpaceAction in [cbsCancelIfPos0NoRepeat, cbsCancelIfNewlyOpenedOrPos0NoRepeat]) and
              (scfAfterKeyUp in FFlags)
           ) )
        then begin
          if Assigned(OnCancel) then OnCancel(Self);
        end
        ...
        if (Length(CurrentString) > 0) then begin
  ...
  Include(FFlags, scfHasEdit);
  Exclude(FFlags, scfAfterKeyUp);
...
procedure TSynBaseCompletionForm.KeyUp(var Key: Word; Shift: TShiftState);
begin
  inherited KeyUp(Key, Shift);
  Include(FFlags, scfAfterKeyUp);
end;
...
function TSynBaseCompletion.GetBackSpaceAction: TSynCompletionBackSpaceAction;
begin
  Result := Form.BackSpaceAction;
end;
...
procedure TSynBaseCompletion.SetBackSpaceAction(AValue: TSynCompletionBackSpaceAction);
begin
  Form.BackSpaceAction := AValue;
end;
